                ...
                <div class="glass rounded-2xl p-5 sm:p-6 shadow-sm">
                    <div class="flex items-center justify-between mb-4">
                        <div class="flex items-center gap-2">
                            ...
                            <h2 class="text-sm font-display font-semibold uppercase tracking-wide">Camera Scanner</h2>
                        </div>
    ...
    <script>
        ...
        presentBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                if (res.ok) {
                    console.log(data);
                    showAlert('Present', `${matchedStudent.name} marked present.`,'success','{{ route("sessions.index") }}');
                    setTimeout(resetToWaiting, 1200);
                } else {
                    switch (res.status) {
                        case 400:
                            showAlert('Error', data.message || 'Bad request.', 'error');
                            break;

                        case 401:
                            showAlert('Unauthorized', data.message || 'Please login first.', 'error');
                            break;

                        case 403:
                            showAlert('Forbidden', data.message ||
                                'You are not allowed to perform this action.', 'error');
                            break;

                        case 404:
                            showAlert('Not Found', data.message || 'Attendance session not found.', 'error');
                            break;

                        case 409:
                            showAlert('Duplicate', data.message || 'Attendance already submitted.', 'warning','{{ route("sessions.index") }}');
                            break;

                        case 422:
                            showAlert('Validation Error', data.message || 'Invalid request data.', 'error');
                            break;

                        case 500:
                            showAlert('Server Error', data.message || 'Internal server error.', 'error');
                            break;

                        default:
                            showAlert(
                                'Error',
                                data.message || `Unexpected error (${res.status}).`,
                                'error'
                            );
                    }
                }
